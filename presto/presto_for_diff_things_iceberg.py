...
conn = presto.connect(host='localhost', port=8080, username='root', catalog='iceberg', schema='default')
cursor = conn.cursor()
...
def replace_table_names(query, suffix):
    ...
    for table in parser.tables:
        new_table_name = f"{table}_{suffix}_iceberg"
...
if results:
    csv_path = f"/mnt/data/presto_results/iceberg_presto_query_execution_stats_{args.suffix}_{args.query_type if args.query_type else 'all'}.csv"
