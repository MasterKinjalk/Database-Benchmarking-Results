...
def replace_table_names(query):
    ...
    for table in parser.tables:
        new_table_name = f"realworld_{table}_parquet_gzip"
...
def main(json_filename, query_type, query_index):
    """Main function to load a specific query from a JSON file, execute it, and log the results."""
    spark = (
        SparkSession.builder.appName("Execute Specific SQL Query")
        .enableHiveSupport()
        .getOrCreate()
    )
    ...
    if query_type in queries and len(queries[query_type]) > query_index:
        ...
        print(query)
        execution_time = execute_query(spark, query)
...
if __name__ == "__main__":
    ...
    main(args.json_filename, args.query_type, args.query_index)
